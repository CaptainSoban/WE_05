...
bool Initialization(){
	...
	*/
	...
	cl_uchar2 ScrPos[122];
	BufIdx[6]=clCreateBuffer(Context,CL_MEM_READ_WRITE,sizeof(cl_uchar2)*122,ScrPos,NULL);
	//
	clSetKernelArg(Planarization,0,sizeof(cl_mem),(void*)&BufIdx[0]);
	clSetKernelArg(Planarization,1,sizeof(cl_mem),(void*)&BufIdx[1]);
	clSetKernelArg(Planarization,2,sizeof(cl_mem),(void*)&BufIdx[2]);
	clSetKernelArg(Planarization,3,sizeof(cl_mem),(void*)&BufIdx[3]);
	clSetKernelArg(Planarization,4,sizeof(cl_mem),(void*)&BufIdx[4]);
	...
	BufIdx[8]=clCreateBuffer(Context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(cl_ushort4)*84,DltIdxIn,NULL);

	cl_bool Polar[220];
	BufIdx[9]=clCreateBuffer(Context,CL_MEM_READ_WRITE,sizeof(cl_bool)*220,Polar,NULL);

	clSetKernelArg(Polarization,0,sizeof(cl_mem),(void*)&BufIdx[0]);
	...
	for(unsigned int a=0;a<800;a++){
		for(unsigned int b=0;b<600;b++){
			PixVectIn[b*800+a].x=PixVect[a][b].x;
			PixVectIn[b*800+a].y=PixVect[a][b].y;
			PixVectIn[b*800+a].z=PixVect[a][b].z;
		}
	}
	BufIdx[10]=clCreateBuffer(Context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(cl_float4)*480000,PixVectIn,NULL);
	
	cl_int *TopDltIdxIn=(cl_int*)malloc(480000*sizeof(cl_int));
	BufIdx[11]=clCreateBuffer(Context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(cl_int)*480000,TopDltIdxIn,NULL);
	
	clSetKernelArg(Hierarchization,0,sizeof(cl_mem),(void*)&BufIdx[6]);
	clSetKernelArg(Hierarchization,1,sizeof(cl_mem),(void*)&BufIdx[0]);
	clSetKernelArg(Hierarchization,2,sizeof(cl_mem),(void*)&BufIdx[2]);
	clSetKernelArg(Hierarchization,3,sizeof(cl_mem),(void*)&BufIdx[4]);
	clSetKernelArg(Hierarchization,4,sizeof(cl_mem),(void*)&BufIdx[5]);
	clSetKernelArg(Hierarchization,5,sizeof(cl_mem),(void*)&BufIdx[8]);
	clSetKernelArg(Hierarchization,6,sizeof(cl_mem),(void*)&BufIdx[10]);
	clSetKernelArg(Hierarchization,7,sizeof(cl_mem),(void*)&BufIdx[9]);
	clSetKernelArg(Hierarchization,8,sizeof(cl_mem),(void*)&BufIdx[11]);
	...
	BufIdx[12]=clCreateBuffer(Context,CL_MEM_READ_WRITE,sizeof(cl_float)*220,Reflection,NULL);

	clSetKernelArg(Hierarchization,0,sizeof(cl_mem),(void*)&BufIdx[12]);
	clSetKernelArg(Hierarchization,1,sizeof(cl_mem),(void*)&BufIdx[8]);
	clSetKernelArg(Hierarchization,2,sizeof(cl_mem),(void*)&BufIdx[7]);
	...
	cl_ushort4 *Bitmap=(cl_ushort4*)malloc(sizeof(cl_ushort4)*480000);
	BufIdx[13]=clCreateBuffer(Context,CL_MEM_READ_WRITE,sizeof(cl_ushort4)*480000,Bitmap,NULL);
	...
	if(status==CL_SUCCESS){
		ConWrite("CODE: CL_SUCCESS. NOTE: Command Queue Created Successfully.\n");
	}
	else{
		switch(status){
		case CL_INVALID_CONTEXT:
			ConWrite("CODE: CL_INVALID_CONTEXT. ERROR: The context is not a valid context.\n");
			break;
		case CL_INVALID_DEVICE:
			ConWrite("CODE: CL_INVALID_DEVICE.\n");
			break;
		case CL_INVALID_VALUE:
			ConWrite("CODE: CL_INVALID_VALUE.\n");
			break;
		case CL_INVALID_QUEUE_PROPERTIES:
			ConWrite("CODE: CL_INVALID_QUEUE_PROPERTIES.\n");
			break;
		case CL_OUT_OF_HOST_MEMORY:
			ConWrite("CODE: CL_OUT_OF_HOST_MEMORY.\n");
			break;
		default:
			ConWrite("Other.\n");
			break;
...
void Draw()
{
	...
	//test
	size_t GlobalThread1[]={128};
	size_t LocalThread[]={8};
	cl_command_queue testcomq=NULL;

	status=clEnqueueNDRangeKernel(testcomq,Planarization,1,NULL,GlobalThread1,LocalThread,0,NULL,NULL);
	a();

	//Planarization
	//size_t GlobalThread1[]={128};
	//size_t LocalThread[]={8};
	status=clEnqueueNDRangeKernel(CommandQueue,Planarization,1,NULL,GlobalThread1,LocalThread,0,NULL,NULL);
	a();
	...
	//Polarization
	//GlobalThread1[]={256};
	status=clEnqueueNDRangeKernel(CommandQueue,Polarization,1,NULL,GlobalThread1,LocalThread,0,NULL,NULL);
	a();
	clFlush(CommandQueue);
	time=0;
	while(CL_SUCCESS!=clFinish(CommandQueue)){
	...
	//Hierarchization
	//GlobalThread1=480000;
	status=clEnqueueNDRangeKernel(CommandQueue,Hierarchization,2,NULL,GlobalThread1,LocalThread,0,NULL,NULL);
	a();
	clFlush(CommandQueue);
	time=0;
	while(CL_SUCCESS!=clFinish(CommandQueue)){
	...
	//Illumination
	//GlobalThread1=220;
	status=clEnqueueNDRangeKernel(CommandQueue,Illumination,1,NULL,GlobalThread1,LocalThread,0,NULL,NULL);
	a();
	clFlush(CommandQueue);
	time=0;
	while(CL_SUCCESS!=clFinish(CommandQueue)){
	...
	//Coloration
	//GlobalThread1=480000;
	status=clEnqueueNDRangeKernel(CommandQueue,Coloration,2,NULL,GlobalThread1,LocalThread,0,NULL,NULL);
	a();
	clFlush(CommandQueue);
	time=0;
	while(CL_SUCCESS!=clFinish(CommandQueue)){
