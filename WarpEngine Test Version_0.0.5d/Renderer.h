...
const char *Cartography[]=
{
	...
	"	"
	"	MapVect[GID]=CpToV[GID]-Scale[GID]*CamNorV1;\n",
	...
	"	ScrPos[GID].x=(int)cos(Theta[GID])*Distance[GID]+CamNorV1.w;\n",
	"	ScrPos[GID].y=(int)sin(Theta[GID])*Distance[GID]+CamNorV2.w;\n",
	"}\n",
	...
	"		if(Polar[i]==true){\n",
	"			v.xy=ScrPos[DltIdx[i].y].xy-ScrPos[DltIdx[i].x].xy;\n",
	"			u.xy=ScrPos[DltIdx[i].z].xy-ScrPos[DltIdx[i].x].xy;\n",
